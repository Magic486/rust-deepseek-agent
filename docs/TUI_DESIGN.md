...
当前实现已经把 Agent 放入 Tokio 后台任务，并通过事件通道实时刷新：

```text
Agent 在执行过程中不断发送 AgentEvent
TUI 收到事件后立即刷新，输入和滚动不再等待网络请求完成
```

当前状态：TUI 已通过后台 Agent 任务和 `handle_user_input_stream` 实现事件级实时刷新。当前还不是 token 级流式输出，模型最终回答仍然等一次 LLM 请求完成后展示。
...
- 支持 Enter 发送
- 支持 Esc/Ctrl+C 退出
- Agent 在后台任务执行，TUI 保持输入和滚动响应

状态：已完成事件级实时刷新；token 级流式输出尚未实现。
...
- 展示当前 Skill
- 展示最近 Memory
- 支持子代理执行流：显示每个子代理的开始、完成、失败，并保留批量任务的真实完成顺序
- 支持任务状态实时更新

状态：已完成 OpenCode 风格骨架、后台 Agent 任务、真实 Todo/Skill/MCP 状态、欢迎页、主工作区、右侧状态栏、底部输入框、消息竖线和工具卡片；Tab agent 切换、Ctrl+P 命令面板和 token 级流式输出尚未实现。
