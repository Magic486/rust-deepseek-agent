...
不要把“已注册”写成“已验证可用”。分别说明：本地工具注册、Skill 已发现、MCP 已配置、搜索密钥是否存在、实际 API/Server 是否成功调用。当前已实现只读子代理的批量并发调度（上限 3），但写操作、命令执行和项目校验仍保持串行；仍未实现 token 级 SSE 流式、向量数据库 RAG、权限确认、可取消任务和隔离工作区时，应在报告和最终说明中明确列为后续工作。
